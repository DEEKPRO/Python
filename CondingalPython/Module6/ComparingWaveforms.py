...
def transcription(data, rate, width):
...
def compare(s1, s2):
    ...
    print(f"🔥 Recording {louder} is longer ({s1['avg_volume']:.1f}s vs {s2['avg_volume']:.1f}s)")
    print("\n💡 In L3, U will control rate and volume when AI speaks!")
...
def main():
    ...
    print("="*40)
    print("Record twic and acompare your voice!")

    audio1, rate, width = record_audio("Recording 1: Speak NORMALLY")
    stats1, text1 = analyze_audio(audio1, rate), transcription(audio1, rate, width)
    ...
    stats2, text2 = analyze_audio(audio2, rate), transcription(audio2, rate, width)
