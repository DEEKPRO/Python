...
def get_essay():
    ...
    essay_type = input("What type of essay are you writing: ")
...
def generate_essay(details):
    ...
    intro_p = f"Write and introduction for and {details['essay_type']} essay about {details['topic']} on the topic of {details['length']}."
    ...
    if choice == 1:
        ...
        print("\n-=-GENERATED STEP-BY-STEP RESPONSE-=-\n")
        print(body_step)
...
def feedback():
    ...
    if rating !=5:
        feedback = input("Provide feedback (tone, structur, etc.): ")
...
def run_activity():
    ...
    if not details["topic"] or not details['essay_type']:
        print("Plese provide at least a topic and essay type to continue.")
        return
