...
SYSTEM_PROMPT = """You are a Math Mastermind. For every math problem: 1) Show step-by-steop solution 2)Explain reasoning 3)Give alternate method if possible 4)Verify anser if possible 5) Use proper notation 6)Break complex problems into parts Format: Problem -> Steps -> **Final Answer** -> Concepts used. Be precise"""
...
def setup_ui():
    ...
    if st.session_state.history:
        c2.download_button("📁 Export", export_txt(st.session_state.history),"Math_Mastermind_Solutions.txt", "text/plains")
    
    with st.form("math_form", clear_on_submit=True):
        q = st.text_area("📐 Enter your math problem here: ", height=100, placeholder="Example: Solve x² + 5x + 6 = 0", key=f"q_{st.session_state.k}")
        a, b = st.columns([3,1])
        ...
        st.markdown("""<style>
                    .box{max-height:500px;overflow-y:auto;border: 5px solid #4CAF50;padding:12px;background: #f7fbff;border-radius:30px}
